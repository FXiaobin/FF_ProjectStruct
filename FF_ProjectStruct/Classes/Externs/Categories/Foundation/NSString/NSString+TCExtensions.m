...
#define PDC_IMAGE_BASE_URL @"http://7xlaxq.com1.z0.glb.clouddn.com/"
...
- (NSString *)imageString {
    ...
    if (![string isUrl] && ![string hasPrefix:@"http"]) {
        string = [PDC_IMAGE_BASE_URL stringByAppendingString:string];
    }
    
    if (![string hasPrefix:PDC_IMAGE_BASE_URL]) {
        return string;
    }
...
-(BOOL)isLeepYear{
    NSInteger year = self.integerValue;
